...
const CONDITIONS: {
...
}[] = [
  {
    ...
    label: 'Above',
    emoji: '🔼',
    description: 'Price will exceed the target',
  ...
  },
  {
    ...
    label: 'Below',
    emoji: '🔽',
    description: 'Price will drop below the target',
  ...
  },
  {
    ...
    label: 'Closes EVEN',
    emoji: '🎲',
    description: 'Last digit of the price will be even',
  ...
  },
  {
    ...
    label: 'Closes ODD',
    emoji: '🎲',
    description: 'Last digit of the price will be odd',
    bgClass: 'bg-violet-500/10',
    borderClass: 'border-violet-500/40',
  },
  {
    value: 'first_to',
    label: 'First to reach…',
    emoji: '🏁',
    description: 'Race: which asset gets there first?',
  ...
  },
  {
    ...
    label: 'Touches price',
    emoji: '💥',
    description: 'Only needs to touch once',
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5">
          Current price: <span className="text-foreground font-medium">${currentAsset.price.toLocaleString()}</span>
        </p>
      </div>

      {/* Condition Selector */}
      <div>
        <label className="text-xs font-medium text-muted-foreground mb-2 block tracking-wider">CONDITION</label>
            ...
            {isRace ? (
              <>
                ...
                <div className="glass-card p-3 rounded-xl space-y-2">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-heading text-muted-foreground tracking-wider">ASSET A — {asset}</span>
                  </div>
                  <input
                    type="number"
                    value={targetPrice}
                ...
                <div className="glass-card p-3 rounded-xl space-y-2">
                  <div className="flex items-center gap-2 mb-1">
                    <Flag className="w-3.5 h-3.5 text-amber-400" />
                    <span className="text-[10px] font-heading text-muted-foreground tracking-wider">ASSET B</span>
                  </div>
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-1.5">
                  <Target className="w-3.5 h-3.5" /> TARGET PRICE
                </label>
                <input
                  type="number"
                  value={targetPrice}
      ...
      <div>
        <label className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5" /> {isDigitBet ? 'EXACT DATE & TIME' : 'DURATION'}
        </label>
        {isDigitBet ? (
          <input
            type="datetime-local"
