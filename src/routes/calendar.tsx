...
export const Route = createFileRoute('/calendar')({
  head: () => ({
    meta: [
      { title: 'Calendar — Anturix' },
      { name: 'description', content: 'Upcoming events and scheduled bets on Anturix.' },
    ],
  }),
...
function CalendarPage() {
  ...
  return (
    <MainLayout>
      <h1 className="text-xl font-heading font-bold text-foreground mb-6">Calendar</h1>

      <div className="space-y-4">
        {events.map(event => {
          const date = new Date(event.date);
          const formattedDate = date.toLocaleDateString('en', { weekday: 'short', day: 'numeric', month: 'short' });
          const formattedTime = date.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' });
          ...
          return (
            <div key={event.id} className="flex items-start gap-4 p-4 rounded-xl bg-card border border-border transition-all hover:border-primary/30">
              <div className="flex flex-col items-center justify-center w-14 h-14 rounded-xl bg-muted">
                ...
                <span className="text-[8px] text-muted-foreground uppercase">{date.toLocaleDateString('en', { month: 'short' })}</span>
              </div>
