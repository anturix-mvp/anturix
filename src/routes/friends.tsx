...
export const Route = createFileRoute("/friends")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Connect with other duelists on Anturix.",
      },
    ],
  }),
    ...
    <MainLayout>
      ...
      <div className="space-y-3">
        {friends.map((user) => (
          <div
            key={user.id}
            className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border transition-all hover:border-primary/30"
          >
            <img
              src={user.avatar}
              alt={user.username}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {user.rank} · {user.wins}W/{user.losses}L · {user.winRate}% WR
              </p>
              {user.streak > 0 && (
