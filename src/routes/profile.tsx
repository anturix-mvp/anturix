...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your Anturix profile, stats, and achievements.",
      },
    ],
  }),
...
const statTooltips: Record<string, string> = {
  "Total Wins":
    "Number of duels and predictions you have won across all game modes",
  "Win Rate":
    "Percentage of total bets you have won. Higher rates unlock better ranks",
  "SOL Earned":
    "Total SOL earned from winning duels, predictions and pool payouts",
  Reputation:
    "Experience points earned from activity. Determines your rank progression",
  "Active Duels":
    "Duels you are currently participating in that haven't been resolved yet",
};
const stats = [
  {
    icon: Trophy,
  ...
  },
  {
    icon: Star,
    label: "Reputation",
    value: currentUser.reputationScore.toLocaleString(),
    color: "text-accent",
  },
  {
    icon: Swords,
    label: "Active Duels",
    value: currentUser.activeDuels,
...
function ProfilePage() {
  const [activeTab, setActiveTab] =
    useState<(typeof profileTabs)[number]>("Achievements");
  ...
  return (
    <MainLayout>
      <div
        className="relative h-32 rounded-xl overflow-hidden mb-16"
        ref={coverRef}
      >
        <motion.div
      ...
      <div className="relative -mt-24 px-4 mb-6">
        <div
          className={`w-24 h-24 rounded-full ${currentUser.rank === "Legend" ? "rank-legend-avatar" : ""}`}
        >
          <img
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
          {stats.map((stat, i) => (
            <Tooltip key={stat.label}>
              ...
              <TooltipContent
                side="bottom"
                className="bg-card border-border text-foreground max-w-[200px]"
              >
      ...
      <div className="flex items-center gap-1 mb-6 border-b border-border overflow-x-auto">
        {profileTabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium transition-all relative whitespace-nowrap ${activeTab === tab ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}
          >
            {tab}
            {activeTab === tab && (
