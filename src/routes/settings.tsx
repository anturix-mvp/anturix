...
type SectionKey = 'profile' | 'wallet' | 'security' | 'notifications' | 'appearance' | 'language' | null;
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="relative">
                <img src={currentUser.avatar} alt="avatar" className="w-16 h-16 rounded-full border-2 border-primary" />
                ...
                {([
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center gap-1.5 p-3 rounded-lg border transition-all ${
                      theme === t.id ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-muted/50 text-muted-foreground hover:border-primary/30'
                    }`}
                  >
            ...
            <div>
              ...
              <div className="flex gap-3">
                {accents.map(a => (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full ${a.color} border-2 ${accentColor === a.id ? 'border-foreground scale-110' : 'border-transparent'} transition-all`}>
                      {accentColor === a.id && <Check className="w-4 h-4 text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />}
                    </div>
            ...
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-lg border transition-all ${
                  language === lang.id ? 'border-primary bg-primary/10' : 'border-border bg-muted/50 hover:border-primary/30'
                }`}
              >
                <span className="text-xl">{lang.flag}</span>
                <span className="text-sm font-medium text-foreground">{lang.label}</span>
                {language === lang.id && <Check className="w-4 h-4 text-primary ml-auto" />}
              </button>
            ))}
            ...
            {group.items.map(item => (
              <div key={item.key}>
                ...
                >
                  ...
                  <motion.div animate={{ rotate: openSection === item.key ? 90 : 0 }} transition={{ duration: 0.2 }}>
